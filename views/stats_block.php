...
<div class="grid grid-cols-2 gap-x-10 gap-y-5 max-w-3xl mx-auto bg-black border-2 border-[#458B41] p-8 font-mono text-green-300 <?= !$lang_stats ? 'mb-[100px]' : '' ?>">
  
    <div class="flex justify-between border-b border-[#458B41] pb-1">
    ...
    <div class="flex justify-between border-b border-[#458B41] pb-1">
      ...
      <span class="font-semibold <?= $next_review_in === 'Due' ? 'text-[coral]' : 'text-green-100' ?>"><?= $next_review_in ?></span>
    </div>

    <div class="flex justify-between border-b border-[#458B41] pb-1" title="<?= $words_count_to_review ? 'In ' . ucwords($words_count_to_review['lang']) : "" ?>">
      <span class="uppercase text-md <?= $words_count_to_review ? 'text-green-700' : 'text-green-400' ?>">Words to Review Now</span>
      <span class="font-semibold text-green-100"><?= $words_count_to_review ? $words_count_to_review['words_to_review'] : '0' ?></span>
    </div>

    <div class="flex justify-between border-b border-[#458B41] pb-1" title="<?= ucwords($langs_added) ?>">
    ...
    <div class="flex justify-between border-b border-[#458B41] pb-1" title="Words that are untested, weak or medium (across all added languages)">
    ...
    <div class="flex justify-between border-b border-[#458B41] pb-1" title="Words that are strong or mastered (across all added languages)">
...
<h3 class="capitalize text-2xl font-mono text-green-600 mb-8 inline-block pb-1 border-b border-b-green-600">Strength distribution per language</h3>
<?php foreach($lang_stats as $lang): ?>
    <div class="mb-[40px]">
        <div class="flex gap-7 border-b border-[#458B41] pb-1">
            <span class="text-green-400 text-md">Language:</span>
            <span class="font-semibold text-green-100"><?= $languages[$lang['language']] ?></span>
            <span class="ml-auto text-gray-500">Total words: <?= $lang['total_words'] ?></span>
        </div>
        <!-- Bar -->
        <div class="flex mt-5 border border-[gray]">
            ...
            <?php if ($lang['medium'] > 0): ?>
                <div class="h-[25px] bg-yellow-900 relative" style="width: <?= floor(((int) $lang['medium'] / (int) $lang['total_words'])*100) ?>%;" title="<?= $lang['medium'] ?> words with strength 'medium'">
