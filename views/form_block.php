<div class="max-w-xl mx-auto mt-12 p-6 bg-black border-2 border-[#458B41] rounded-xl shadow-lg font-mono text-[#458B41] relative mb-10">
  ...
  <form method="POST" action="../public/index.php?action=addword" class="space-y-4">
    <!-- Word / Phrase -->
    <div>
      ...
      <input autofocus="true" name="word" type="text" placeholder="Enter word" class="w-full px-3 py-2 bg-black border border-[#458B41] rounded focus:outline-none focus:ring-2 focus:ring-green-500"
        value="<?= $form_data ? htmlspecialchars($form_data['word']) : null ?>"
      />
    </div>
    ...
    <div>
      ...
      <input type="text" name="translation" placeholder="Enter translation" class="w-full px-3 py-2 bg-black border border-[#458B41] rounded focus:outline-none focus:ring-2 focus:ring-green-500"
        value="<?= $form_data ? htmlspecialchars($form_data['translation']) : null ?>"
      />
    </div>
     ...
     <div class="flex gap-4">
        ...
        <select name="language" class="w-full px-3 py-2 bg-black border border-[#458B41] rounded focus:outline-none focus:ring-2 focus:ring-green-500">
            <?php foreach($languages as $key=>$val): ?>
                <option value="<?= $key ?>" 
                    <?= $form_data && $form_data['language'] === $key ? 'selected' : '' ?>
        ...
        <select name="category" class="w-full px-3 py-2.5 bg-black border border-[#458B41] rounded focus:outline-none focus:ring-2 focus:ring-green-500 text-sm">
            <?php foreach($categories as $key=>$val): ?>
                <option value="<?= $key ?>" 
                    <?= $form_data && $form_data['category'] === $key ? 'selected' : '' ?>
    ...
    <div>
      ...
      <textarea name="example" placeholder="Optional sentence" class="w-full min-h-[42px] h-[42px] max-h-[100px] px-3 py-2 bg-black border border-[#458B41] rounded focus:outline-none focus:ring-2 focus:ring-green-500"><?= $form_data ? htmlspecialchars($form_data['example']) : '' ?></textarea>
    </div>
    ...
    <div>
      ...
      <textarea name="notes" placeholder="Optional hint" class="w-full min-h-[42px] h-[42px] max-h-[100px] px-3 py-2 bg-black border border-[#458B41] rounded focus:outline-none focus:ring-2 focus:ring-green-500"><?= $form_data ? htmlspecialchars($form_data['notes']) : '' ?></textarea>
    </div>
